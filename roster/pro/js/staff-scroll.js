import {
  ...
  loadAllowedDomains,
  isValidDomain,
} from "https://staffscroll.co.uk/roster/pro/assets/js/staff-scroll-utils.js";
...
// New container where timeline rows/events are rendered
...
async function checkDomainAndLoadData(domain) {
  ...
  // Safe init of zoom and theme listeners
...
function safeInitZoomAndThemeListeners(callback) {
  try {
    // Check if elements needed by initZoomAndThemeListeners exist
    // For example, if initZoomAndThemeListeners expects #zoom-button and #theme-toggle
    const zoomBtn = document.getElementById("zoom-button");
    const themeToggle = document.getElementById("theme-toggle");
    
    if (!zoomBtn || !themeToggle) {
      console.warn("Zoom or Theme controls not found. Skipping initZoomAndThemeListeners.");
      return;
    }

    initZoomAndThemeListeners(callback);
  } catch (e) {
    console.warn("initZoomAndThemeListeners failed:", e);
  }
}
...
// Called whenever an event is changed, now will be called from drag/resize listeners
function onEventChanged(updatedEvent) {
  // Track modified event for saving
  trackModifiedEvent(updatedEvent);
  // Update currentStaff data for UI sync
  updateCurrentStaffEvent(updatedEvent);
}

// Update the currentStaff array to reflect the changed event immediately
...
// New function: Set up listeners for your drag/drop/resize events (adjust as needed)
function setupEventChangeListeners() {
  // Example: listen for custom event 'eventDragEnd' on timelineRowsContainer
